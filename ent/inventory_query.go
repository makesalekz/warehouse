...
func (iq *InventoryQuery) QueryWarehouse() *WarehouseQuery {
	query := (&WarehouseClient{config: iq.config}).Query()
...
func (iq *InventoryQuery) QueryItems() *InventoryItemQuery {
	query := (&InventoryItemClient{config: iq.config}).Query()
	query.path = func(ctx context.Context) (fromU *sql.Selector, err error) {
		if err := iq.prepareQuery(ctx); err != nil {
			return nil, err
		}
...
func (iq *InventoryQuery) Clone() *InventoryQuery {
	...
	return &InventoryQuery{
		...
		withWarehouse: iq.withWarehouse.Clone(),
...
func (iq *InventoryQuery) WithWarehouse(opts ...func(*WarehouseQuery)) *InventoryQuery {
	query := (&WarehouseClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
	iq.withWarehouse = query
	return iq
}

// WithItems tells the query-builder to eager-load the nodes that are connected to
// the "items" edge. The optional arguments are used to configure the query builder of the edge.
func (iq *InventoryQuery) WithItems(opts ...func(*InventoryItemQuery)) *InventoryQuery {
	query := (&InventoryItemClient{config: iq.config}).Query()
	for _, opt := range opts {
		opt(query)
	}
...
func (iq *InventoryQuery) sqlAll(ctx context.Context, hooks ...queryHook) ([]*Inventory, error) {
	...
	if query := iq.withWarehouse; query != nil {
		if err := iq.loadWarehouse(ctx, query, nodes, nil,
			func(n *Inventory, e *Warehouse) { n.Edges.Warehouse = e }); err != nil {
			return nil, err
		}
	}
	if query := iq.withItems; query != nil {
		if err := iq.loadItems(ctx, query, nodes,
			func(n *Inventory) { n.Edges.Items = []*InventoryItem{} },
			func(n *Inventory, e *InventoryItem) { n.Edges.Items = append(n.Edges.Items, e) }); err != nil {
			return nil, err
		}
	}
	return nodes, nil
...
func (iq *InventoryQuery) loadWarehouse(ctx context.Context, query *WarehouseQuery, nodes []*Inventory, init func(*Inventory), assign func(*Inventory, *Warehouse)) error {
...
func (iq *InventoryQuery) loadItems(ctx context.Context, query *InventoryItemQuery, nodes []*Inventory, init func(*Inventory), assign func(*Inventory, *InventoryItem)) error {
	...
	if len(query.ctx.Fields) > 0 {
		query.ctx.AppendFieldOnce(inventoryitem.FieldInventoryID)
	}
	query.Where(predicate.InventoryItem(func(s *sql.Selector) {
		s.Where(sql.InValues(s.C(inventory.ItemsColumn), fks...))
	}))
	neighbors, err := query.All(ctx)
	if err != nil {
		return err
...
func (iq *InventoryQuery) querySpec() *sqlgraph.QuerySpec {
	...
	if fields := iq.ctx.Fields; len(fields) > 0 {
		...
		if iq.withWarehouse != nil {
			_spec.Node.AddColumnOnce(inventory.FieldWarehouseID)
		}
	}
